...
static void					display_info(t_env *z)
{
	if (z->display_info)
	{
	 	mlx_string_put(z->mlx_ptr, z->win, 15, 725, 0x00000000, "Fractal       : ");
	 	if (z->frac_type == 1)
	 	...
	 	mlx_string_put(z->mlx_ptr, z->win, 15, 745, 0x00000000, "Zoom          : ");
	 	mlx_string_put(z->mlx_ptr, z->win, 195, 745, 0x00000000, ft_itoa(z->zoom));
	 	mlx_string_put(z->mlx_ptr, z->win, 15, 765, 0x00000000, "Itération(s)   : ");
